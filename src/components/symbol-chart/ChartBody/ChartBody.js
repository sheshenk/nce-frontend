...
const ChartBody = ({ symbol, interval, type }) => {
	...
	const { loading: subscriptionloading, error: subscriptionError, data: subscriptionData } = useSubscription(MARKET_HISTORY_SUBSCRIPTION, {
		variables: { symbol }
	})
	...
	useEffect(() => {
		if (subscriptionData !== undefined) finchartOnMessage(subscriptionData.updateMarketHistory, interval, setFinchartData)
